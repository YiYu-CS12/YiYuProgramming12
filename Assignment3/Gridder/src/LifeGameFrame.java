...
public class LifeGameFrame extends javax.swing.JFrame 
    ...
    private void jButton1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton1ActionPerformed
        ...
        for (int i = 0; i < 100; i++) {
            int column = random.nextInt(gridCount);
    ...
    private void jButton7ActionPerformed(java.awt.event.ActionEvent evt) {
        ...
        for (int row = 1; row < 99; row++) {
            for (int col = 1; col < 99; col++) {
                if (grid[col][row] == 0) {
                    int whiteCount = 0;
                    if (grid[col][row - 1] == 1) whiteCount++;
                    if (grid[col][row + 1] == 1) whiteCount++;
                    if (grid[col - 1][row] == 1) whiteCount++;
                    if (grid[col + 1][row] == 1) whiteCount++;
                    if (grid[col - 1][row - 1] == 1) whiteCount++;
                    if (grid[col + 1][row - 1] == 1) whiteCount++;
                    if (grid[col - 1][row + 1] == 1) whiteCount++;
                    if (grid[col + 1][row + 1] == 1) whiteCount++;
                    if (whiteCount >= 2) {
    ...
    private void jButton8ActionPerformed(java.awt.event.ActionEvent evt) {
        ...
        for (int row = 0; row < gridCount; row++) {
            for (int col = 0; col < gridCount - 1; col++) {
                temp[col][row] = grid[col + 1][row];
            }
        }
        for (int row = 0; row < gridCount; row++) {
            temp[gridCount - 1][row] = grid[0][row];
        }
        for (int row = 0; row < gridCount; row++) {
            for (int col = 0; col < gridCount; col++) {
                grid[col][row] = temp[col][row];
    ...
    private void jButton9ActionPerformed(java.awt.event.ActionEvent evt) {
        ...
        for (int row = 0; row < gridCount; row++) {
            for (int col = 0; col < gridCount; col++) {
                temp[col][gridCount - 1 - row] = grid[row][col];
            }
        }
        for (int row = 0; row < gridCount; row++) {
            for (int col = 0; col < gridCount; col++) {
